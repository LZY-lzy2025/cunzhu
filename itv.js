export default {
  async fetch(request, env, ctx) {
    ...
    // --- 2. 获取订阅 ---
    if (path === '/sub' || path === '/sub/m3u') {
      return await generateUserM3U(request, env, url);
    }
    if (path === '/sub/txt') {
      return await generateUserTXT(request, env, url);
    }
    if (path === '/sub/tvbox') {
      return await generateUserTVBox(request, env, url);
    }
...
// === 生成 TXT 订阅 ===
async function generateUserTXT(request, env, url) {
  const token = url.searchParams.get('token');
  if (!token) return new Response('Missing Token', { status: 401 });
...
// === 生成 TVBox JSON 订阅 ===
async function generateUserTVBox(request, env, url) {
  const token = url.searchParams.get('token');
  if (!token) return new Response('Missing Token', { status: 401 });
  ...
  const origin = url.origin;
  // 直接引用刚才生成的 TXT 接口，这是 TVBox 能够读取的最佳格式
  const txtUrl = `${origin}/sub/txt?token=${token}`;
  ...
  return new Response(JSON.stringify(tvboxConfig, null, 2), { 
...
function renderUserDashboard(username) {
  ...
  '    button.copy-btn { padding: 4px 10px; font-size: 12px; margin-right: 5px; border-radius: 4px; }\n' +
  '    button.btn-m3u { background: #3b82f6; }\n' +
  '    button.btn-txt { background: #10b981; }\n' +
  '    button.btn-tvbox { background: #8b5cf6; }\n' +
  ...
  '        let m3uLink = origin + \'/sub?token=\' + t.token;\n' +
  '        let txtLink = origin + \'/sub/txt?token=\' + t.token;\n' +
  '        let tvboxLink = origin + \'/sub/tvbox?token=\' + t.token;\n' +
  '        html += \'<tr>\' +\n' +
  '          \'<td>\' + t.token + \'</td>\' +\n' +
  '          \'<td>\' + limitTxt + \'</td>\' +\n' +
  '          \'<td>\' + t.expireText + \'</td>\' +\n' +
  '          \'<td>\' +\n' +
  '            \'<button class="copy-btn btn-m3u" onclick="copy(\\\'\' + m3uLink + \'\\\')">M3U</button>\' +\n' +
  '            \'<button class="copy-btn btn-txt" onclick="copy(\\\'\' + txtLink + \'\\\')">TXT</button>\' +\n' +
  '            \'<button class="copy-btn btn-tvbox" onclick="copy(\\\'\' + tvboxLink + \'\\\')">TVBox</button>\' +\n' +
  '          \'</td>\' +\n' +
